...
|---------|------|---------|------|
| v9 | 2026-09-18 | **A fee added from this screen can be split between renters, like the one added from the Add Additional Fee page.** **FR 18 and FR 19 are corrected.** *Raised by the user 2026-09-18, asking why the two screens do not match.* **v2 made the difference deliberate and it had outlived its reason.** FR 19 said the fee *"carries no `tenantIds`, so it is charged to every active tenant"* and that the panel *"shall say so and link to the screen that can charge a subset"* — a reasonable division of labour when the tenant picker lived on that other page, and an odd one once you notice both screens call the **same** endpoint, `POST /rent/agreements/{id}/additional-charges`, with the same `tenantShares` field on the body. This screen simply never filled it in. **Spec `02` v8 moved the split editor into the shared fee panel**, behind a `tenants` input, so a host that hands the panel a roster gets the editor and a host that does not renders no renter control at all. **This screen now hands it one.** FR 18 therefore loads the roster (`GET /rent/agreements/{id}/tenants`) **alongside** the lease rather than after it, so the fee step still renders once instead of filling in over two visible stages. A `204` — the lease exists but step 2 was never saved — becomes an **empty** roster rather than none: this screen does author who pays, the lease just has nobody on it yet, and the editor says so. A lease that does not exist fails on the first step as before, and the roster read is dropped with it. **The pointer to the other screen goes**, because there is nothing left to point at. **No backend change, no contract change** — the same body, from one more screen. | — |
...
18. **v2, corrected in v9** — On a valid id the system shall load the lease
    (`GET /rent/agreements/{id}`) **and its saved renters (`GET /rent/agreements/{id}/tenants`)
    concurrently**, and only then render `AdditionalChargePanelComponent`, passing the lease's
    `propertyOwnerId`, `startDate`, `endDate`, derived month-to-month invoice count, **the roster and
    whether the lease bills as a group**. An unknown lease shall be reported on the first step rather
    than advancing, and the roster read is dropped with it.
    **Concurrently, and the roster is required, not optional.** The roster is what makes the panel offer
    its split editor at all (spec `02` v8), so fetching it after the lease would render the fee step
    twice — once without a way to say who pays, then again with one. A `204` becomes an **empty** roster
    rather than none: this screen authors who pays even when the lease has nobody saved yet.
19. **v2, corrected in v9** — On the panel's `created` event the system shall
    `POST /rent/agreements/{id}/additional-charges` once, keep the panel open until the server answers,
    and render a `422` without discarding the authored fee.
    ~~The fee carries no `tenantIds`, so it is charged to every active tenant; the panel shall say so and
    link to the screen that can charge a subset.~~ **Reversed in v9.** The fee may now be **split between
    renters** from this screen, through the same editor the Add Additional Fee page uses (spec `02` v8):
    the charge carries `tenantShares` when the owner splits it and omits the field when they leave it
    shared. The link to the other screen is removed, since that screen no longer does anything this one
    cannot. **A fee left shared behaves exactly as it did** — no `tenantShares`, every active renter
    owes it — so nothing about the previous behaviour is lost, only the inability to choose otherwise.
