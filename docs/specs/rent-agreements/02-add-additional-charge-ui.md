...
24. **v11** — When the owner switches the split's unit, the page shall **carry every row's figure
    across** into the new unit and shall **disclose it when the amounts change as a result**, naming
    what moved.
    **The money does move, and no rounding rule prevents it.** An even `$300` across three renters is
    `100.00` each (requirement 17 divides the money). Expressed as percentages that is `33.33` each,
    and `33.33 %` of `300` is `99.99` — so the rows become `99.99 / 100.01 / 100.00` the moment the
    unit changes. `100.00 / 100.00 / 100.00` **has no expression as three percentages of `300`**;
    that is precisely why backend requirement 30 was reversed in its v105 to divide the money rather
    than the percentage.
    **So the page tells the owner instead of pretending.** `setShareUnit` currently claims in its own
    comment that the figure is carried across *"so what the renter owes does not move"*. It moves by a
    cent, silently, and the comment is what stopped anyone looking. A disclosure beside the control —
    naming the rows whose amount changed and by how much — is what makes the choice an informed one.
