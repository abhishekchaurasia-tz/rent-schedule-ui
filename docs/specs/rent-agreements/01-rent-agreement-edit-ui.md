...
15. **v24** — The system shall let a tester supply an access token for the environment the build
    ...
    **The confusing part, stated so nobody has to find it:** once a token is present the gateway
    derives its own caller headers from it. The three ids in the box and the token can therefore
    describe **different people**, and what the backend records is the gateway's answer, not the
    box's. The screen should say so when both are set.
