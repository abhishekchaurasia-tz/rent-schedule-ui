...
|---------|------|--------|---------|------|
| v21 | 2026-09-14 | Abhishek Chaurasia | **The two ids that say *who is saving* stop travelling in the lease body and become request headers — and until this ships with its backend half, no lease can be created at all.** New **requirement 12**. *Requested by the user 2026-09-14, as the client half of `innago-rent-accounting` spec `01-rent-agreement.md` v89–v90 (FR-127, FR-128).* The Billing API now requires **`OrganizationId`** and **`PropertyOwnerId`** on `POST /rent/agreements`, and **no longer reads `propertyOwnerId` from the request body**. **What changes here.** `CreateRentAgreementRequest` loses `propertyOwnerId` (`rent-agreement.models.ts`); the create form gains an **`accountId` control seeded with `crypto.randomUUID()`**, exactly as `propertyUnitId`/`propertyId`/`propertyOwnerId` already are — the account is new to this application, which has no sign-in and no notion of one; and a new **HTTP interceptor** attaches both headers, registered through `provideHttpClient(withInterceptors(...))` in `app.config.ts`, which today registers none at all. **Why an interceptor rather than the one service method that needs it** (*chosen by the user 2026-09-14*): it is the seam a bearer token will eventually use, so the wiring survives the change that retires these headers. The cost is stated rather than discovered — an interceptor stamps both headers on **every** call to the API, including the invoicing and line-item requests that neither need nor read them. That is harmless (ASP.NET Core ignores unknown headers) but it is real, so the interceptor is scoped to `environment.apiBaseUrl` and sends nothing when no scope has been published. **The interceptor cannot read the form**, so the values reach it through a small injectable the create component publishes to — the one piece of new structure this change adds, and the reason requirement 12 has a part (c). **`propertyOwnerId` stays a form control:** `AdditionalChargePanelComponent` and the line-item pickers bind to it for catalog scoping (component lines 739, 752), and the edit page patches it from the loaded agreement (line 442). It stops being a *body field*; it does not stop existing. **Release coupling, stated plainly:** this change and the backend's are one release. Ship this first and every create is refused by a server that still wants the body field; ship the backend first and every create is refused for the missing header. Nothing is half-saved in either direction — the create screen is simply unusable until both are out. | [2026-09-14T1400-01-the-scope-ids-travel-as-headers](../../plans/rent-agreements/2026-09-14T1400-01-the-scope-ids-travel-as-headers.md) |
...
it is now the `PropertyOwnerId` header above (requirement 12a, backend FR-128). The backend ignores
