...
18. **v4** — The system shall bind the item catalog from the **income-list** payload, sending
    `isFromIncomeList=true` and `isHOATerm=true` — the pair the backend requires before it admits
    `HOAFee`.
19. **v4** — The picker shall offer "+ Add Item Type" on a non-deposit invoice, taking a **name** and a
    **kind of charge**, resolving it through `POST /api/v1/line-items` (get-or-create by name), adding
    the resolved entry to the local catalog, and selecting it on the row. It shall send `itemType` in
    snake_case and read the returned PascalCase form back.
