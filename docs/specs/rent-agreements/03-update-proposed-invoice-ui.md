...
21. **v6** — The system shall refuse to remove a **deposit** invoice's last deposit-shaped line
    (`Deposit` or `PetDeposit`, matching the backend's `DepositTypes`), reporting it inline and issuing
    no request. It shall still permit removing that invoice's other lines, and shall not apply the rule
    to a rent invoice — where dropping the rent line is a supported edit, because rent is optional on a
    proposal. Backend spec `07` FR 37.
