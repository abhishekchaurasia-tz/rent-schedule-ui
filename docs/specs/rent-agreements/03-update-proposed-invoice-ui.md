...
|---------|------|---------|------|
| v7 | 2026-09-11 | **The rent line stops being removable too, and it turns out to be the same rule as the deposit — following backend spec `07` v11.** *Reported by the user: the screen still offered it and it still went.* v6 guarded only the deposit because v10 of the backend spec recorded rent as unprotected; v11 reverses that, on the narrower reading of the instruction it came from — rent is optional when a charge invoice is **created**, not deletable from one that bills it. So `isLastDepositLine` becomes **`isLastSubjectLine`**: it asks the invoice what it exists to bill (`Deposit`/`PetDeposit` on a deposit invoice, `Rent` on anything else) and refuses only the last such line, which is one branch rather than two and matches how the backend states it through `InvoiceLineRank`. **Retention, not possession** — the rule binds a set that *already* carries a subject line, so an invoice billing only fees is untouched, and R5 survives without an exception written for it. FR 21 amended. | [2026-09-11T1200-the-ui-refuses-before-the-server-does](../../plans/rent-agreements/2026-09-11T1200-the-ui-refuses-before-the-server-does.md) |
| v6 | 2026-09-11 | **Two refusals move from after submit to before it, following backend spec `07` v10.** That version added two rules this screen could trip without warning. (1) **FR 37 — a deposit invoice must keep a deposit line.** The emptiness guard here does not cover it: a deposit invoice may legitimately carry a `Credit` beside its deposit, so dropping the deposit leaves a set that is *not* empty, sails past "an invoice must keep at least one line", and leaves a deposit invoice billing no deposit. The server now refuses that with `422 invoice.deposit_line_may_not_be_removed` — and the backend spec's decision D5 is explicit that the API refuses **and** the UI does not offer the control, of which this screen had only the first half. New **FR 21**. (2) **FR 35 — a line's amount is never zero.** This screen validates `quantity >= 0.01` and `rate >= 0.01`, which are exactly the input guards the backend had before v10 and exactly what lets `0.01 × 0.40 = 0.004` round to `$0.00` with both fields legal. The check has to read the product, after rounding toward zero at two places, the way the server does. New **FR 22**. **Nothing was broken before this** — both cases already failed safely, because `describeError` renders the RFC 9457 `detail` and the server's messages are legible — so what changes is learning while typing rather than on submit. **Nothing was needed for the ordering half of `07` v10**, and that was worth checking rather than assuming: this app sorts invoice lines nowhere (no `.sort` or `orderBy` under `src/app` at all), so the order the API now guarantees reaches the screen untouched, and no request carries a position. | [2026-09-11T1200-the-ui-refuses-before-the-server-does](../../plans/rent-agreements/2026-09-11T1200-the-ui-refuses-before-the-server-does.md) |
| v5 | 2026-09-01 | **The "+ Add Item Type" row is now the fee panel's one-field row, not a two-field form.** v4 asked for a name *and* a classification, which no other screen asks for — reported by the user with side-by-side screenshots. It is one name box and an Add button, and the new entry is filed under `InvoiceItemType.Miscellaneous`: the same type `AdditionalChargeItemProvisioningService.CustomItemType` assigns when the additional-fee route get-or-creates an entry from a typed name, so both paths classify a custom item identically rather than by coincidence. The classification picker and its `PICKABLE_ITEM_TYPES` list are deleted. The row itself lives in `NewItemTypeFormComponent`, used only by this page — the fee panel keeps its own inline row, unchanged — and `.item-picker-new-row` moves to `src/styles.scss` so one rule styles both. **Also corrected: the catalog fetch no longer sends `isHOATerm=true`.** v4 added it so the list would actually widen, but it is a claim about the lease that this screen cannot make; only `isFromIncomeList=true` is sent now. FR 18 and 19 amended. | [2026-09-01T1200-invoice-edit-custom-line-item](../../plans/rent-agreements/2026-09-01T1200-invoice-edit-custom-line-item.md) |
| v4 | 2026-09-01 | **The item picker gains "+ Add Item Type", and the catalog is bound from the income-list payload.** v2 left the affordance out because this endpoint parses `itemType` into a fixed `InvoiceItemType` enum and refuses anything else — true, but it does not follow that a custom item is impossible. It is created in the **catalog**: `POST /api/v1/line-items` is an idempotent get-or-create by name, so a custom item here is a custom **name** filed under an existing classification, and the line then carries the resulting `lineItemId` plus that classification — both of which the correction endpoint accepts. The picker asks for the two things the catalog needs, a name and a kind of charge, and selects the resolved entry on the row (seeding the description like any other pick, per v3). **The create body sends `itemType` in snake_case while the catalog returns it PascalCase** — the request binds to a C# enum so the API's `SnakeCaseLower` converter reads it, the response is a plain `string` from `ToString()` that the converter never sees; both forms are modelled explicitly rather than derived. Never offered on a **deposit** invoice, where a catalog row must be system-defined. The catalog fetch now sends `isFromIncomeList=true`, and only that — `isHOATerm` is a claim about the *lease* (that its terms are HOA terms) which this screen has no way to know, so it is not asserted; whether `HOAFee` items come back is then the backend's rule rather than a guess made here. New FR 18–20. Also extracts the shared `.item-picker-*` chrome into `src/styles.scss`, which cleared this file's and the fee panel's per-component budget warnings. | [2026-09-01T1200-invoice-edit-custom-line-item](../../plans/rent-agreements/2026-09-01T1200-invoice-edit-custom-line-item.md) |
...
17. **v3** — Choosing a catalog item shall copy that item's name into the row's description **only
    ...
    alone.
18. **v4** — The system shall bind the item catalog from the **income-list** payload, sending
    `isFromIncomeList=true` and **not** `isHOATerm`, which is a claim about the lease this screen
    cannot make.
19. **v4, amended v5** — The picker shall offer "+ Add Item Type" on a non-deposit invoice as a
    **single name box**, matching the fee panel, resolving it through `POST /api/v1/line-items`
    (get-or-create by name) filed under `Miscellaneous` — the type the additional-fee route itself
    assigns to a custom item — then adding the entry to the local catalog and selecting it on the row.
    It shall send `itemType` in snake_case and read the returned PascalCase form back.
20. **v4** — The system shall not offer "+ Add Item Type" on a **deposit** invoice, where a catalog row
    must be system-defined, and shall enforce that in the component and not only by hiding the button.
21. **v6, widened v7** — The system shall refuse to remove the last line of the invoice’s own
    **subject kind** — `Deposit` or `PetDeposit` on a deposit invoice, `Rent` on anything else —
    reporting it inline and issuing no request. It shall still permit removing that invoice’s other
    lines. Stated as **retention**: the rule binds an invoice that *already* carries such a line, so a
    charge-only invoice that never billed rent is untouched by it. Backend spec `07` FR 37 and 39.
22. **v6** — The system shall refuse to submit while any line's **quantity × rate, rounded toward zero
    at two decimal places**, is zero, naming the offending rows and issuing no request. Requirement 14's
    per-field minimums cannot see this: both inputs clear them and only the product rounds away.
    Backend spec `07` FR 35.
